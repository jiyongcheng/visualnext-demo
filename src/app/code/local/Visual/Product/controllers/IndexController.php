...
class Visual_Product_IndexController extends Mage_Core_Controller_Front_Action
{
    public function indexAction()
    {
        echo "Hello World";

        $simpleProductId = $this->createSimpleProduct();
        $this->createCProduct($simpleProductId);
    }
...
//        Mage::app()->setCurrentStore(Mage_Core_Model_App::ADMIN_STORE_ID);
        ...
        $sku = 'sku-'.rand(1,1000);
        $productName = 'simple product '.$sku;
        $weight = 4;
        $manufacturerOptionId = $this->getOptionId('manufacturer','China');
        $colorOptionId = $this->getOptionId('color','red');
        $country = 'AF';
        $price = '22.22';
        $cost = '11.11';
        $specialPrice= '0.44';
...
//    ->setUpdatedAt(strtotime('now')) //product update time
                ...
                ->setManufacturer($manufacturerOptionId) //manufacturer id
                ->setColor($colorOptionId)
...
//                ->setSpecialToDate('06/30/2014') //special price to (MM-DD-YYYY)
            ...
            $simpleProduct = $this->addProductImages($simpleProduct);
            $simpleProduct->save();
            /** EXTERNAL IMAGE IMPORT - END **/

            $simpleProductId = $simpleProduct->getId();
            echo 'success<>'.$simpleProductId;
    ...
    public function addProductImages($product)
    {
        $sku = $product->getSku();
        $images = array(
            'thumbnail'=>'http://pic.nipic.com/2007-11-09/2007119122637650_2.jpg',
            'image'=>'http://img1.3lian.com/img013/v3/56/d/101.jpg',
...
//            );
     ...
     * @return bool
     */
    public function createConfigurableProductOld($simpleProductId)
    {
        return false;
...
//                ->setSpecialToDate('06/30/2014') //special price to (MM-DD-YYYY)
            ...
            if ($attr->usesSource()) {
                $greenOptionId = $attr->getSource()->getOptionId("red");
            }
            $configurableProductsData[$simpleProductId] = array( //['920'] = id of a simple product associated with this configurable
                '0' => array(
                    'label' => 'Green', //attribute label
                    'attribute_id' => $colorAttributeId, //attribute ID of attribute 'color' in my store
                    'value_index' => $greenOptionId, //value of 'Green' index of the attribute 'color'
    ...
    public function createCProduct($simpleProductId)
    {
